<template>
	<view style="padding-bottom: 100rpx;">
		<view class="order_nav_view">
			<view class="bgc_fff order_nav_box ">
				<vanTabs v-model="tabIndex" :tabLine="false" :columns="tabColumns" @update:modelValue="changeTabs">
				</vanTabs>
			</view>
		</view>
		<view style="padding-bottom: 30rpx;background-color:#f2f2f2">
			<template v-for="(item,index) in orderList" v-if="orderList.length>0">
				<order-goods-card @logistics="catLogistics" @tapCard="tapCard" :orderData="item"
					@reload="reloadOrder"></order-goods-card>
			</template>
		</view>
		<view class="flexc-jsa" style="margin:30rpx 0" v-if="!orderList.length>0">
			<image src='../../static/assets/png/no-order.png' class="empty-view "></image>
			<text class="font_44 empty-order-text">暂无订单</text>
		</view>

	</view>
</template>

<script setup>
	import orderSearch from './components/order-search.vue'
	import orderGoodsCard from './components/order-goods-card.vue'
	import vanTabs from '/component/sp/vanTabs'
	import {
		getAllOrder,
		getUnPayOrder,
		getUnSendOrder,
		getUnReceiveOrder,
		getHasReceiveOrder
	} from '/api/order.js'
	import {
		onMounted,
		ref,
		onBeforeUnmount
	} from 'vue'
	import {
		onLoad
	} from '@dcloudio/uni-app'
	import {
		useUserInfo
	} from '/store/user.js'
	const store = useUserInfo();
	const props = defineProps({
		tab: {
			type: Number,
			default: 0
		}
	})
	let tabIndex = ref(0)
	//传入的方法
	const orderList = ref([])
	const tabColumns = ref([{
			title: '全部',
			id: '4'
		}, {
			title: '待付款',
			id: '5'
		}, {
			title: '待发货',
			id: '6'
		}, {
			title: '待收货',
			id: '7'
		},
		{
			title: '已收货',
			id: '8'
		}
	])
	const tapCard = () => {
		uni.navigateTo({
			url: '/pages/order/orderDetail'
		})
	}
	const catLogistics = () => {
		uni.navigateTo({
			url: '/pages/logistics/logistics'
		})
	}

	function reloadOrder() {
		console.log("刷新目标index" + tabIndex.value)
		getOrderByIndex(tabIndex.value)
	}
	const changeTabs = (index) => {
		console.log('选中的tab索引变化了', index);
		//执行传入vanTabs内的消息改变
		//根据不同的index获取不同的表更新list栏，然后传入card的循环列表更新订单和对应的按钮逻辑
		getOrderByIndex(index)
	}
	const getOrderByIndex = async (index) => {
		console.log(index)
		if (index == 0) {
			const res = await getAllOrder({
				user_id: store.user_id
			})
			orderList.value = res.data
			console.log(res.data)
		} else if (index == 1) {
			const res = await getUnPayOrder({
				user_id: store.user_id
			})
			orderList.value = res.data
			console.log(res.data)
		} else if (index == 2) {
			const res = await getUnSendOrder({
				user_id: store.user_id
			})
			orderList.value = res.data
			console.log(res.data)
		} else if (index == 3) {
			const res = await getUnReceiveOrder({
				user_id: store.user_id
			})
			orderList.value = res.data
			console.log(res.data)
		} else if (index == 4) {
			const res = await getHasReceiveOrder({
				user_id: store.user_id
			})
			orderList.value = res.data
			console.log(res.data)
		}
		console.log(orderList.value)
	}
	onMounted(() => {
		console.log('tab' + props.tab)
		tabIndex.value = props.tab
		getOrderByIndex(tabIndex.value)
		uni.$on('reload', () => {
			getOrderByIndex(tabIndex.value)
		})
	})
	onBeforeUnmount(() => {
		// 在 unMounted 时使用 uni.$off 解除监听
		uni.$off('reload', () => {
			getOrderByIndex(tabIndex.value);
		});
	})
</script>

<style scoped>
	.empty-order-text {
		font-weight: 800;
		text-align: center;
		margin: 25rpx 0;
	}

	page {
		background-color: #f2f2f2;
	}

	.order_nav_view {
		width: 100%;
		height: 84rpx;
	}

	.order_nav_box {
		padding-top: 20rpx;
		width: 100%;
		height: 84rpx;
		/* #ifdef H5 */
		top: var(--window-top);
		/* #endif */
		/* #ifndef H5 */
		top: 0;
		/* #endif */
		z-index: 1;
		left: 0;
		right: 0;
	}
</style>